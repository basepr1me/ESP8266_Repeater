...
CHIP ?= esp8266
ESP_ROOT ?= $(PREFIX)/espressif/$(CHIP)
...
# LIB locations
ARDUINO_ROOT = $(PREFIX)/arduino
ARDUINO_LIBS = ${ARDUINO_ROOT}/libraries
LOCAL_LIBS = ./
CUSTOM_LIBS = /home/basepr1me/Arduino/libraries
...
ifneq ($(ESPTOOL),)
	...
	ifeq ($(CHIP),esp8266)
		ESPTOOL_COM = $(ESPTOOL)
		UPLOAD_COM = $(ESPTOOL_PATTERN) --before no_reset --after soft_reset write_flash 0x00000 $(BUILD_DIR)/$(MAIN_NAME).bin
		FS_UPLOAD_COM = $(ESPTOOL_PATTERN) --before no_reset --after soft_reset write_flash $(SPIFFS_START) $(FS_IMAGE)
	endif
endif
...
ESP_ARDUINO_VERSION = $(shell pkg_info | grep arduino-$(CHIP) | cut -d' ' -f1)
ifeq ($(CHIP),esp8266)
	XTENSA = lx106
else
	XTENSA = esp32
endif
XTENSA_VERSION := $(shell pkg_info | grep xtensa-$(XTENSA)-elf | cut -d' ' -f1)
...
ifeq ($(LIBS),)
	...
	LIBS := $(shell perl -e 'use File::Find;@d = split(" ", shift);while (<>) {$$f{"$$1"} = 1 if /^\s*\#include\s+[<"]([^>"]+)/;}find(sub {if ($$f{$$_}){print $$File::Find::dir," ";$$f{$$_}=0;}}, @d);' \
													"$(CUSTOM_LIBS) $(LOCAL_LIBS) $(ESP_LIBS) $(ARDUINO_LIBS)" $(SKETCH) $(call find_files,S|c|cpp,$(SKETCH_DIR)))
endif

IGNORE_PATTERN := $(foreach dir,$(EXCLUDE_DIRS),$(dir)/%)
USER_INC := $(filter-out $(IGNORE_PATTERN),$(call find_files,h,$(SKETCH_DIR) $(dir $(LIBS))))
USER_SRC := $(SKETCH) $(filter-out $(IGNORE_PATTERN),$(call find_files,S|c|cpp, $(LIBS)))
...
ifeq ($(IGNORE_STATE),)
	...
	STATE_INF := $(strip $(foreach par,$(CMD_LINE),$(if $(findstring =,$(par)),$(par),))) \
							 $(XTENSA_VERSION) $(ESP_ARDUINO_VERSION)
...
$(MAIN_EXE): $(CORE_LIB) $(USER_OBJ)
	...
	$(LINK_PREBUILD)
	echo "	Versions: $(XTENSA_VERSION), $(ESP_ARDUINO_VERSION)"
	echo 	'#include <buildinfo.h>' >$(BUILD_INFO_CPP)
	echo '_tBuildInfo _BuildInfo = {"$(BUILD_DATE)","$(BUILD_TIME)","$(XTENSA_VERSION)","$(ESP_ARDUINO_VERSION)"};' >>$(BUILD_INFO_CPP)
...
dump_flash:
	...
	$(ESPTOOL_PATTERN) --before no_reset --after soft_reset read_flash 0 $(shell perl -e 'shift =~ /(\d+)([MK])/ || die "Invalid memory size\n";$$mem_size=$$1*1024;$$mem_size*=1024 if $$2 eq "M";print $$mem_size;' $(FLASH_DEF)) $(FLASH_FILE)

dump_fs:
	echo Dumping flash file system to directory: $(FS_REST_DIR)
	-$(ESPTOOL_PATTERN) --before no_reset --after soft_reset read_flash $(SPIFFS_START) $(SPIFFS_SIZE) $(FS_IMAGE)
	mkdir -p $(FS_REST_DIR)
	echo
	echo == Files ==
	$(RESTSPIFFS_COM)
...
restore_flash:
	...
	$(ESPTOOL_PATTERN) --before no_reset --after soft_reset -a soft_reset write_flash 0 $(FLASH_FILE)

erase_flash:
	$(ESPTOOL_PATTERN) --before no_reset --after soft_reset erase_flash
